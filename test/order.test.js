...
const MetaSaltMarketV2 = artifacts.require("MetaSaltMarketV2.sol");
const LibOrderTest = artifacts.require("LibOrderTest.sol");
const TestERC721 = artifacts.require("TestERC721.sol");
const TestERC1155 = artifacts.require("TestERC1155.sol");
const TestRoyaltyEngine = artifacts.require("TestRoyaltyEngine.sol");
...
contract("MetaSaltMarket", accounts => {			
	...
	beforeEach(async () => {
		...
		royaltyEngine = await TestRoyaltyEngine.new();
		...
		testing = await MetaSaltMarketV2.new();
		testing.__MetaSaltMarket_init(10, "0xfcC6f84bF615B0A2001B2e73315d285745Fb3821", erc721Test.address, erc1155Test.address);		
	...
	it("ERC721Lazy Testing", async () => {
		...
		const left = Order(creator, Asset(ERC721_LAZY, encodedMintData, 1), ZERO, Asset(ETH, "0x", "1000"), "0x");		
		const right = Order(accounts[2], Asset(ETH, "0x", "1000"), ZERO, Asset(ERC721_LAZY, encodedMintData, 1),  "0x");
		...
		let account3_balance = await web3.eth.getBalance("0xfcC6f84bF615B0A2001B2e73315d285745Fb3821");
		console.log('account_balance', account1_balance, account2_balance, account3_balance);
		let address = await testing.owner();
		console.log('address', address);
		await testing.setRoyaltyRegister(royaltyEngine.address);
		try {
			const res = await testing.matchOrders(left, signatureLeft, right, signatureRight, { from: accounts[2], value: "1000", gasPrice: 20000000000 });
		} catch (err) {
			console.log('err' ,err);
		}
		...
		// const leftSecond = Order(accounts[2], Asset(ERC721_LAZY, encodedMintDataSecond, 1), ZERO, Asset(ETH, "0x", "1000"), "0x");		
		// const rightSecond = Order(accounts[3], Asset(ETH, "0x", "1000"), ZERO, Asset(ERC721_LAZY, encodedMintDataSecond, 1),  "0x");
		// let signatureLeftSecond = await getSignature(leftSecond, accounts[2]);
		// let signatureRightSecond = await getSignature(rightSecond, accounts[3]);
		// await erc721Test.setApprovalForAll(testing.address, true, {from: accounts[2]});	

		// try {
		// 	await testing.matchOrders(leftSecond, signatureLeftSecond, rightSecond, signatureRightSecond, { from: accounts[3], value: "1000", gasPrice: 20000000000 });
		// } catch (err){
		// 	console.log('err' ,err);
		// }
